...
- Only limited months of congestion data have been ingested so far — if a user asks for a trend over time, check what months actually exist rather than implying full coverage. The ingested month (May 2026) was chosen as a neutral one, outside both the summer and winter-holiday peaks. Seasonal bias largely cancels inside a comparison set because every airport in it is measured in the same month, but it does not cancel for airports whose own demand is seasonal — ski and summer-resort airports such as BGR, BTV and MHT may be measured between their seasons. Raise this when a ranking turns on one of them.
